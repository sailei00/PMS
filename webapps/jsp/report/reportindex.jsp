...
<c:if test="${not empty reportList}">
		$(function() {
			...
			if(department == '综采队'){
				...
				$("#ratio1").val(0.66);
				//$("#ratio2").val(2.99);
				$("#ratio2").val(2.76);		//2016年下半年系数调整
				//$("#ratio3").val(0.86);
				$("#ratio3").val(0.54);		//2016年下半年系数调整
			}
			if(department == '机电机运队'){
				// $("#ratio1").val(0.65);
				$("#ratio1").val(0.53);		//2016年下半年系数调整
				// $("#ratio2").val(2.41);		
				$("#ratio2").val(1.21);		//2016年下半年系数调整		
				// $("#ratio3").val(0.32);
				$("#ratio3").val(0.14);		//2016年下半年系数调整
			}
			if(department == '综掘一队') {
				$("#drivingtype").show(0).change();
				// $("#ratio2").val(0.9);
				$("#ratio2").val(0.66);		//2016年下半年系数调整
				$("#ratio4type").change();
			}
			if(department == '综掘二队') {
				$("#drivingtype").show(0).change();
				// $("#ratio2").val(0.5);
				$("#ratio2").val(0.22);			//2016年下半年系数调整
				$("#ratio4type").change();
			}
			if(department == '机修厂') {
				$("#ratio1").val(0);
		...
		function calc(){
			...
			if(money4 != '' && money4 != null) {
				...
				var balance4;		//超支/节余
				if($.isNumeric(ratio4)){
					if($("#ratio4type").val()=="isMining") {
						result4 = Number(mine * ratio4).toFixed(2);
					}
					if($("#ratio4type").val()=="isDriving") {
						result4 = Number(length * ratio4).toFixed(2);
					}
					result4 = Number(mine * ratio4).toFixed(2);
					$("#result4").val(result4);
		...
		function selectDrivingOrMining(obj) {
			...
			if(obj.value == 'isDriving') {
					// ratio4.val('509');
					if(department == '综掘一队') {
						ratio4.val('198');			//2016年下半年系数调整
					}
					if(department == '综掘二队') {
						ratio4.val('204');			//2016年下半年系数调整
					}
			}
			...
			<table align="center"  width="60%" border=1>
				...
				<c:forEach items="${reportList}" var="li">
					<tr>
						...
						<td align="center"><font color="${li.checkAmount?'green':'red'}">${li.checkAmount?"√":"×"}</font></td>
					</tr>
				</c:forEach>
			</table>
			<br/>
